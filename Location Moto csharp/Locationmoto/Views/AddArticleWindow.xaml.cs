...
{
    /// <summary>
    /// Logique d'interaction pour AddMotoWindow.xaml
    /// </summary>
    public partial class AddMotoWindow : Window
    {
        private readonly MotoVM motoVM;
        private readonly CategorieVM categorieVM;
        private string selectedImagePath;

        /// <summary>
        /// Initialise une nouvelle instance de la fenêtre d'ajout de moto
        /// avec les ViewModels nécessaires.
        /// </summary>
        /// <param name="motoVM">Le ViewModel des motos</param>
        /// <param name="categorieVM">Le ViewModel des catégories</param>
        public AddMotoWindow(MotoVM motoVM, CategorieVM categorieVM)
        {
            InitializeComponent();
            this.motoVM = motoVM;
        ...
        /// <summary>
        /// Gère le clic sur le bouton "Ajouter" pour créer une nouvelle moto.
        /// Valide les entrées utilisateur, crée une nouvelle moto et la sauvegarde dans la base de données.
        /// </summary>
        /// <param name="sender">Le bouton qui déclenche l'événement</param>
        /// <param name="e">Les données associées à l'événement de clic</param>
        private void Ajouter_Click(object sender, RoutedEventArgs e)
        {
            if (string.IsNullOrWhiteSpace(NomMotoTextBox.Text))
            {
                MessageBox.Show("Veuillez saisir un nom de moto.", "Erreur", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            if (motoVM.AjouterMoto(nouvelleMoto, selectedImagePath))
            {
                MessageBox.Show("Moto ajoutée avec succès.", "Succès", MessageBoxButton.OK, MessageBoxImage.Information);
                Close();
            }
        }
        ...
        /// Ferme la fenêtre sans sauvegarder de moto.
        /// </summary>
        /// <param name="sender">Le bouton qui déclenche l'événement</param>
        /// <param name="e">Les données associées à l'événement de clic</param>
        ...
        {
            e.Handled = !Regex.IsMatch(e.Text, @"^[0-9]$"); 
           //est utilisée pour contrôler l'entrée de texte dans un champ de saisie (comme un TextBox
        }
    }
}
